...
def normalize_depth_withMask(depth_image, depth_mask, percentile = 99, cutoff = None):
    masked_depth = depth_mask * depth_image
    max_valid = np.percentile(masked_depth,percentile)

    if cutoff:
        if max_valid > cutoff:
            max_valid = cutoff

    normalized_depth = (depth_image - np.min(depth_image)) / max_valid * 255
...
class DIODE_Depth(DIODE):
    def __init__(self, meta_fname, data_root, splits, scene_types, transform = None):
        super().__init__(meta_fname, data_root, splits, scene_types)
        self.transform = transform
        return
    
    def __getitem__(self, index):
        image,depth_image,depth_mask = super().__getitem__(index)
        normalized_depth = normalize_depth_withMask(depth_image=depth_image,depth_mask=depth_mask)

        normalized_depth = torch.from_numpy(normalized_depth.T)

        image = normalized_depth.repeat(3, 1, 1)
...
if __name__ == "__main__":
    ...
    dataset = DIODE(meta_fname=meta_name,
                    data_root=dataset_path,
                    splits='val',
                    scene_types=['outdoor', 'indoors'])
    image, depth_image, depth_mask = dataset[0]
    normalized_depth = normalize_depth_withMask(depth_image=depth_image,depth_mask=depth_mask)
    pass